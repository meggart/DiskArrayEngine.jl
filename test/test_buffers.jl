@testset "Output buffer collections" begin
  ...
  outspecs = create_outwindows((7, 4), dimsmap=(1, 2, 3), windows=([1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6, 7, 7], [1, 1, 1], 1:4))
  ...
  k1 = DiskArrayEngine.BufferIndex((1:1, 1:1, 1:2))
  @test haskey(obc.buffers,k1)
  ob = obc.buffers[k1]
  @test ob isa DiskArrayEngine.OutArrayBuffer
  @test ob.nwritten[] == 1
  @test ob.ntot == 6
  @test DiskArrayEngine.getloopinds(ob) == (1, 2, 3)
  @test ob.offsets == (0, 0, 0)
  @test ob.a == zeros(Int32, 2, 1, 2)
  @test bufview1 === ob
  ...
  @test outar[1, 1, :] == [1, 1, 0, 0]
  @test all(iszero, outar[2:end, 1, :])
  
  bufview2 = extract_outbuffer(lr[3,1,1],outspecs,init,buftype,obc)
  k2 = DiskArrayEngine.BufferIndex((2:3, 1:1, 1:2))
  ob = obc.buffers[k2]
  @test ob.nwritten[]==1
  @test ob.ntot==3
  ob.a .= 2
