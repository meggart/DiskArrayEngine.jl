...
function ProductArray(members::Tuple)
    ...
    return ProductArray{et,length(members),typeof(members)}(members)
end
...
function Base.getindex(a::ProductArray{<:Any,N}, i::Vararg{Int,N}) where {N}
    return getindex.(a.members, i)
end
...
function RegularWindows(start, stop; window=1, step=window)
    return RegularWindows(start, stop, step, window)
end
...
function Base.getindex(r::RegularWindows, i::Int)
    ...
    return i0:min((i0 + r.window - 1), r.stop)
end
...
struct LoopIndSplitter{TR,NTR,BACK} end
threadinds(::LoopIndSplitter{TR}, lr) where {TR} = map(Base.Fix1(getindex, lr), TR)
function nonthreadinds(::LoopIndSplitter{<:Any,NTR}, lr) where {NTR}
    return map(Base.Fix1(getindex, lr), NTR)
end
...
function LoopIndSplitter(nd, reddims::Tuple)
    ...
    return LoopIndSplitter{nonreddims,reddims,back}()
end
...
function merge_loopranges_threads(i_tr::CartesianIndex, i_ntr::CartesianIndex, lspl)
    b = get_back(lspl)
    return CartesianIndex(map(b) do (is_tr, i)
        if is_tr
            i_tr.I[i]
        else
            i_ntr.I[i]
