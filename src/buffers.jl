...
windowbuffersize(looprange, window) =
...
DiskArrays.haschunks(c::EmptyInput) = DiskArrays.Unchunked()
DiskArrays.eachchunk(c::EmptyInput) = DiskArrays.GridChunks(map(UndefinedChunks, c.s))
DiskArrays.element_size(c::EmptyInput{T}) where {T} = sizeof(Base.nonmissingtype(T))
