...
function getoutspec(r::GMWOPResult{<:Any,<:Any,<:Any,<:Any,ISPEC}) where {ISPEC}
    return r.op.outspecs[ISPEC]
end
...
function results_as_diskarrays(o::GMDWop; cs=nothing, max_cache=1e9)
    ntuple(length(o.outspecs)) do i
        ...
        GMWOPResult{T,N,typeof(o),typeof(cs),i}(
            o, Val(i), cs, max_cache, size(outspec.lw.windows)
        )
    end
end
...
function compute!(
    ret, a::DiskArrayEngine.GMWOPResult; runner=LocalRunner, threaded=true, kwargs...
)
    lr = DiskArrayEngine.optimize_loopranges(
        a.op, 5e8; tol_low=0.2, tol_high=0.05, max_order=2
    )
    ...
    run(r)
    return fetch(outars[iout])
end
function compute(
    a::DiskArrayEngine.GMWOPResult; runner=LocalRunner, threaded=true, kwargs...
)
    return compute!(nothing, a; runner, threaded, kwargs...)
end
