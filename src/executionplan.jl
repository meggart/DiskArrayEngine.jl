...
function Base.show(io::IO, ::MIME"text/plain", p::ExecutionPlan)
    ...
    return printinfo(io, p; extended=!comp)
end
...
function actual_time_per_array(p::ExecutionPlan)
    input_times = map(p.input_chunkspecs) do chunkspec
        ...
        prod(actual_chunk_access.(chunkspec.cs, mylr, chunkspec.lw.windows.members)) *
        repfac *
        chunkspec.sr
    end
    output_times = map(p.output_chunkspecs) do chunkspec
...
function printinfo(io::IO, plan::ExecutionPlan; extended=true)
    ...
    for (ii, ia) in enumerate(plan.input_chunkspecs)
        ...
        println(
            io, "With factor : $(arf.input_times[ii]) resulting in $(tpa.input_times[ii])"
        )
    ...
    for (ii, ia) in enumerate(plan.output_chunkspecs)
        ...
        println(
            io, "With factor : $(arf.output_times[ii]) resulting in $(tpa.output_times[ii])"
        )
...
function apparent_chunksize(inar::InputArray)
    cs = eachchunk(inar.a)
    lwm = inar.lw.windows
    return DiskArrays.GridChunks(apparent_chunksize.(cs.chunks, lwm.members))
end
...
function apparent_chunksize(cs, lw)
    ...
    return DiskArrays.chunktype_from_chunksizes(l)
end
...
function get_chunkspec(outspec, ot)
    ...
    si = map(
        m -> last(inner_range(last(m))) - first(inner_range(first(m))) + 1,
        outspec.lw.windows.members,
    )
    ...
    return (; cs, app_cs, sr, lw, elsize, windowfac, windowoffset, repfac)
end
...
function avg_step(lw, ::Union{Increasing,Decreasing}, ::Any)
    return length(lw) > 1 ? mean(diff(first.(inner_range.(lw)))) : 1.0
end
...
function optimize_loopranges(
    ...
    optprob = OptimizationFunction(
        compute_time, Optimization.AutoForwardDiff(); cons=all_constraints!
    )
    ...
    return ExecutionPlan(
        input_chunkspecs, output_chunkspecs, (sol.u...,), totsize, sol.objective, lr
    )
end
...
function custom_loopranges(op, steps::Tuple)
    ...
    input_chunkspecs = get_chunkspec.(op.inars, (totsize,))
    output_chunkspecs = get_chunkspec.(op.outspecs, op.f.outtype)
    length(steps) == length(totsize) ||
        error("Steps for loop ranges does not fit number of loop vars ($(length(totsize)))")
    chunkspecs = (input_chunkspecs..., output_chunkspecs...)
...
function kgv(i...)
    ...
    return prod((i) -> first(i)^last(i), merge(max, f...))
end
kgv(i) = i

function is_possible_candidate(cand, smax, optires, reltol_low, reltol_high)
    reltol = cand > optires ? reltol_high : reltol_low
    return cand <= smax && abs(cand - optires) / optires < reltol
end

function find_adjust_candidates(
    optires, smax, intsizes; reltol_low=0.2, reltol_high=0.05, max_order=2
)
    ...
    if optires > smallest_common
        for ord in 1:max_order
            ...
            is_possible_candidate(cand, smax, optires, reltol_low, reltol_high) &&
                return cand
        end
    ...
    if length(intsizes) > 1
        ...
        return find_adjust_candidates(
            optires, smax, Base.front(intsizes); reltol_low, reltol_high, max_order
        )
    end
...
function adjust_loopranges(
    optotal, approx_opti; tol_low=0.2, tol_high=0.05, max_order=2, force_regular=false
)
    ...
    r = map(approx_opti, 1:length(approx_opti), optotal.windowsize) do sol, iopt, si
        ...
        if !isempty(inaxchunks)
            ...
            cands = find_adjust_candidates(
                sol, si, insizes; reltol_low=tol_low, reltol_high=tol_high, max_order
            )
        ...
        newbreaks = map(mylr, outspec.lw.windows.members) do breaks, window
            if get_overlap(window) isa Repeating
                ...
                for i in 1:(length(r) - 1)
                    ...
                    isplit = split_orig
                    r1_array, r2_array = window[first(r1)]:window[last(r1)],
                    window[first(r2)]:window[last(r2)]
...
function output_chunks(p::ExecutionPlan)
    return output_chunks.(p.output_chunkspecs, (p.lr,))
end
