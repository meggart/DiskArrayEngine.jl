...
struct WindowGroup{P}
    ...
    function WindowGroup(parent, g::UnitRange{Int})
        ...
        return new{typeof(parent)}(parent, g)
    end
end
function inner_range(w::WindowGroup)
    return first(inner_range(w.parent[first(w.g)])):last(inner_range(w.parent[last(w.g)]))
end
...
function compute_overlap(r::AbstractVector{<:WindowGroup}, ordering)
    return compute_overlap(inner_range.(r), ordering)
end
...
function compute_ordering(r)
    ...
    return ordering = if allsorted(exts)
        Increasing()
...
function to_window(r)
    eltype(r) <: Int ||
        eltype(r) <: AbstractUnitRange{Int} ||
        eltype(r) <: WindowGroup ||
        throw(ArgumentError("Windows must contain Ints or UnitRanges as elements"))
    ordering = compute_ordering(r)
    overlap = compute_overlap(r, ordering)
    sparsity = compute_sparsity(r)
    return Window(r, ordering, overlap, sparsity)
end
...
function Base.getindex(m::MovingWindow, i::Int)
    return (m.first + (i - 1) * m.steps):(m.first + (i - 1) * m.steps + m.width - 1)
end
...
function get_sparsity(w::MovingWindow)
    return abs(w.steps) > 2 * w.width ? Sparse(w.width / abs(w.steps)) : Dense()
end
