...
function getinterpinds(oldvals::AbstractRange, newvals::AbstractRange)
    return (newvals .- first(oldvals)) ./ step(oldvals) .+ 1
end
function getinterpinds(r1, r2)
    rev = if issorted(r1)
        false
    elseif issorted(r1; rev=true)
        true
    else
        error("Axis values are not sorted")
    end
    map(r2) do ir
...
function getallsteps(xcoarse, xfine)
    ...
    resin = DiskArrayEngine.MovingWindow(
        floor(Int, first(interpinds)), 1, 2, length(resout)
    )
    return interpinds, resin, resout
end
...
function interpolate_block!(xout, data, nodes...; dims, threaded=false)
    ...
    return fill_nodes!(itp, xout, outnodes)
end
@noinline function fill_nodes!(itp, xout, outnodes)
    pa = ProductArray(outnodes)
    return broadcast!(Base.splat(itp), xout, pa)
end
...
function interpolate_diskarray!(aout, a, conv)
    ...
    lr = DiskArrayEngine.optimize_loopranges(
        r.op, 5e8; tol_low=0.2, tol_high=0.05, max_order=2
    )
