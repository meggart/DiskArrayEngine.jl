...
function windows_from_spec(::ReduceAggregator, groups, sdim)
    length(groups) == sdim || throw(
        ArgumentError(
            "Length of group vector must be equal to size of the respective dimension"
        ),
    )
    groupid, n = rle(groups)
    allunique(groupid) ||
        throw(ArgumentError("Aggregation to cyclic groups not yet implemented"))
    return 1:sdim, outrepfromrle(n)
end

function windows_from_spec(::ReduceAggregator, windowsize::Int, sdim)
    return 1:sdim, [((i - 1) ÷ windowsize) + 1 for i in 1:sdim]
end
...
function windows_from_spec(::DirectAggregator, windowsize::Int, sdim)
    return MovingWindow(1, windowsize, windowsize, ceil(Int, sdim / windowsize)),
    1:ceil(Int, sdim / windowsize)
end
...
function gmwop_for_aggregator(agg, dimspec, inar; ismem=false, outchunks=nothing)
    ...
    outspecs = create_outwindows(
        length.(outwindows); windows=outwindows, ismem=ismem, chunks=outchunks
    )
