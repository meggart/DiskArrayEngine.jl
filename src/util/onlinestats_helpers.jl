...
function fit_online!(xout, x, f=identity)
    fx = f(x)
    return ismissing(fx) || OnlineStats.fit!(xout[], fx)
end
fin_online(x) = OnlineStats.nobs(x) == 0 ? missing : OnlineStats.value(x);
function disk_onlinestat(
    s::Type{<:OnlineStats.OnlineStat}, rt=typeof(OnlineStats.value(s())), preproc=identity
)
    return create_userfunction(
        fit_online!,
        rt;
        is_mutating=true,
...
const func_to_online = Dict([
    ...
    maximum => (
        DerivedOnlineStat{
            OnlineStats.Extrema,v -> last(OnlineStats.value(v)),OnlineStats.fit!
        },
        Union{Float64,Missing},
    ),
    minimum => (
        DerivedOnlineStat{
            OnlineStats.Extrema,v -> first(OnlineStats.value(v)),OnlineStats.fit!
        },
        Union{Float64,Missing},
    ),
])
