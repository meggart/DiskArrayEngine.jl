...
function replace_dimids(lw::LoopWindows, dimidmap)
    ...
    return LoopWindows(lw.windows, Val(newlr))
end
...
function possible_breaks(dg::DimensionGraph, inode::Int)
    ...
    return possible_breaks(inwindows, outwindows)
end
...
function possible_breaks(inwindows, outwindows)
    ...
    while endval <= maxval
        ...
        if allequal(endval_candidates_in) &&
            allequal(endval_candidates_out) &&
...
function BlockFunctionChain(f::Union{ElementFunction,BlockFunction}, info)
    ...
    return BlockFunctionChain(funcs, args, transfers)
end
...
function BlockFunctionChain(conn::MwopConnection)
    nlr = max(
        maximum(maxlr, conn.inwindows; init=0), maximum(maxlr, conn.outwindows; init=0)
    )
    return BlockFunctionChain(
        conn.f, (length(conn.inwindows), length(conn.outwindows), nlr)
    )
end
...
function run_block(
    f::BlockFunctionChain, ref_inow, inbuffers_wrapped, outbuffers_now, threaded
)
    func = first(f.funcs)
    args = first(f.args)
            ...
            if ifrom in outbuffers_written && !(ifrom in transfers_done)
                ...
                for ito in itos
                    ...
                    if size(ib.a) != size(ob.a)
                        broadcast!(
                            ob.finalize, view(ib.a, Base.OneTo.(size(ob.a))...), ob.a
                        )
                    else
...
function get_groups(windows, strategies)
    return Dict(
        filter(
            !isnothing,
            map(windows.windows.members, strategies, getloopinds(windows)) do w, strat, il
...
function blockwindows_inconn(inconn, strategies, i_eliminate)
    ...
    return newinwindows, newoutwindows
end
...
function _blockwindows(windows, groups)
    ...
    return LoopWindows(ProductArray(newwindows), windows.lr)
end
...
function merged_connection(
    ::Type{DirectMerge}, _, inconn, outconn, i_eliminate, newop, strategy, dimidmap
)
    ...
    return MwopConnection(
        newinputids, outconn.outputids, newop, newinwindows, newoutwindows
    ),
    []
end

function merged_connection(
    ::Type{BlockMerge}, nodegraph, inconn, outconn, i_eliminate, newop, strategy, dimmap
)
    inconninwindows, inconnoutwindows = blockwindows_inconn(inconn, strategy, i_eliminate)
    outconninwindows, outconnoutwindows = blockwindows_outconn(
        outconn, strategy, i_eliminate
    )
