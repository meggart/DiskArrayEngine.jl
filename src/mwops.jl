...
function purify_window(lw::LoopWindows)
    ...
    return LoopWindows(ProductArray(pw), lw.lr)
end
...
function InputArray(
    a::AbstractArray; dimsmap=ntuple(identity, ndims(a)), windows=Base.OneTo.(size(a))
)
    length(dimsmap) == ndims(a) || throw(
        ArgumentError("number is dimensions in loop dimension map not equal to ndims(a)"),
    )
    length(windows) == ndims(a) ||
        throw(ArgumentError("number of supplied loop windwos not equal to ndims(a)"))
    lw = LoopWindows(ProductArray(to_window.(windows)), Val((dimsmap...,)))
    return InputArray(a, lw)
end
...
function create_outwindows(
    s;
    dimsmap=ntuple(identity, length(s)),
    windows=Base.OneTo.(s),
    chunks=map(_ -> nothing, s),
    ismem=false,
)
    ...
    if sort(collect(dimsmap)) != 1:length(dimsmap)
        throw(
            ArgumentError(
                "Vanishing dimensions in outputs are not allowed. Please add length-1 dummy dimensions for every input axis.",
            ),
        )
    end
    return (; lw=LoopWindows(outrp, Val((dimsmap...,))), chunks, ismem)
end
...
function GMDWop(inars, outspecs, f)
    ...
    foreach(outspecs) do spec
        ...
        sort(collect(li)) == 1:nd ||
            throw(ArgumentError("Vanishing Dimensions are not allowed for output arrays."))
    end
    return GMDWop(inars, outspecs, f, s, lspl)
end
    ...
    map(plan.output_chunkspecs, op.f.outtype) do outspec, rettype
        ...
        if sizeof(rettype) * prod(outsize) > 1e8
            zcreate(
                ...
                outsize...;
                path=tempname(),
                fill_value=typemin(retnmtype),
                chunks=chunksize,
                fill_as_missing=Missing <: rettype,
            )
        else
            if par_only
