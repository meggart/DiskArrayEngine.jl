...
function innercode(cI, f, xin, xout)
...
function run_block(op::GMDWop, loopRanges, xin, xout, threaded)
    ...
    return run_block(op.f.f, loopRanges, xin, xout, lspl)
end
...
function run_block(f::BlockFunction, loopRanges, xin, xout, lspl)
    ...
    return _run_block(f, myinwork, myoutwork, isa(lspl, LoopIndSplitter))
end
function _run_block(f::BlockFunction{<:Any,Mutating}, myinwork, myoutwork, threaded::Bool)
    return f.f(myoutwork..., myinwork...; dims=getdims(f), threaded=threaded)
end
function _run_block(
    f::BlockFunction{<:Any,NonMutating}, myinwork, myoutwork, threaded::Bool
)
...
function LocalRunner(
    ...
    cb, runfilter = callback_and_runfilters(
        loopranges, showprogress, restartfile, restartmode
    )
    return LocalRunner(
        op, loopranges, outars, threaded, inbuffers_pure, outbuffers, cb, runfilter
    )
end
...
function default_loopbody(
    inow, op, inbuffers_pure, outbuffers, threaded, outars, cb, runfilter
)
    @debug "inow = ", inow
    if all(c -> need_run(inow, c), runfilter)
        inbuffers_wrapped = read_range.((inow,), op.inars, inbuffers_pure)
        outbuffers_now =
            extract_outbuffer.((inow,), op.outspecs, op.f.init, op.f.buftype, outbuffers)
...
@noinline function run_loop(
    ...
    return notify_callback(cb, nothing)
end
...
function PMapRunner(
    ...
    all(isnothing, op.f.red) ||
        error("PMapRunner can not be used for reductions. Use DaggerRunner instead")
    loopranges = plan_to_loopranges(exec_plan)
    inbuffers_pure = generate_inbuffers(op.inars, loopranges)
    outbuffers = generate_outbuffers(op.outspecs, op.f, loopranges)
    cb, runfilter = callback_and_runfilters(
    ...
    cbc = if !isempty(cb)
        channel = Distributed.RemoteChannel(
            () -> Channel{Union{Nothing,eltype(loopranges)}}(), 1
        )
        @async while true
            update = take!(channel)
    ...
    return PMapRunner(
        op, loopranges, outars, threaded, inbuffers_pure, outbuffers, cbc, runfilter
    )
end
